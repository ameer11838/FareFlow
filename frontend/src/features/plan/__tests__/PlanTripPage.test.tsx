...
describe('PlanTripPage — taking a journey', () => {
  ...
  it('runs select, start, track, end, and server-priced payment without sending a fare', async () => {
    ...
    const activeSheet = screen.getByTestId('transit-session-sheet')
    expect(activeSheet).toHaveAttribute('aria-modal', 'false')
    expect(activeSheet.closest('.session-backdrop')).toHaveClass('session-backdrop-active')
    expect(screen.getAllByText(/You are here · Suburban Station/i)).not.toHaveLength(0)
